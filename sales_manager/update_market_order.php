...
if (isset($_POST['submit']) && $order_id) {
    ...
    $sql = "UPDATE Orders SET customer_id=$customer_id, total_amount=$total_amount, status='$status' WHERE order_id=$order_id";

    if ($conn->query($sql) === TRUE) {
        $message = 'Market order updated successfully.';
...
<html lang="en">
  <head>
    ...
    <title>Update Market Order - Sales Manager</title>
    <link rel="stylesheet" href="../style.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" />
  </head>
  <body>
    <?php include '../components/topbar.html'; ?>
    <?php $page_title = 'Update Market Order'; include '../components/header.html'; ?>
    ...
    <main>
      <div class="card" style="max-width: 600px; margin: 0 auto">
        <div class="card-header">
          <span class="card-title">Update Market Order</span>
        </div>
        <?php if ($message): ?>
        ...
        <form action="" method="POST">
          <div class="form-group">
            ...
            <input type="text" id="order_id" value="ORD-<?php echo $order['order_id']; ?>" disabled />
          </div>
          <div class="form-group">
          ...
          <button type="submit" name="submit" class="btn-submit">
            <i class="fa fa-check"></i> Update Market Order
          </button>
        </form>
        <?php else: ?>
        <p>Order not found.</p>
