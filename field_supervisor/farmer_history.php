...
if ($selected_farmer_id) {
    $query = "SELECT b.batch_id, b.batch_number, p.product_name, b.quantity, b.unit, DATE_FORMAT(b.purchase_date, '%Y-%m-%d') AS purchase_date, q.quality_tag FROM Batches b JOIN Products p ON b.product_id=p.product_id JOIN Quality_Checks q ON b.batch_id=q.batch_id WHERE b.farmer_id=$selected_farmer_id AND q.quality_tag='Approved' ORDER BY b.purchase_date DESC";
    $history = $conn->query($query);
    $row = $conn->query("SELECT name FROM Farmers WHERE farmer_id=$selected_farmer_id");
    $farmer_name = $row->num_rows ? $row->fetch_assoc()['name'] : '';
}
?>
...
<html lang="en">
  <head>
    ...
    <title>Sales History - Field Supervisor</title>
  ...
  <body>
    ...
    <main>
      ...
      <div class="card">
        <div class="card-header">
          <span class="card-title">Approved Batches for <?php echo htmlspecialchars($farmer_name); ?></span>
        </div>
        <table>
          <tr>
            <th>Batch ID</th>
          ...
          <tr>
            ...
            <td>
              <button class="btn btn-info" onclick="viewDetails('<?php echo $row['batch_number']; ?>', '<?php echo htmlspecialchars(addslashes($row['product_name'])); ?>', '<?php echo $row['quantity'] . ' ' . htmlspecialchars(addslashes($row['unit'])); ?>', '<?php echo $row['purchase_date']; ?>', 'Approved')">View</button>
            </td>
          </tr>
          <?php }
          } else {
            echo '<tr><td colspan="6">No approved batches found for this farmer.</td></tr>';
          } ?>
        </table>
      </div>
      <?php endif; ?>
    ...
    <script>
      function viewDetails(batch, product, qty, date, status) {
        document.getElementById('modalBody').innerHTML = `
          ...
          <div class="detail-row"><span class="detail-label">Status:</span><span class="detail-value"><span class="status completed">${status}</span></span></div>
